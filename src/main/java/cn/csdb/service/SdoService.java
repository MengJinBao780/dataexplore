...
public class SdoService {
    ...
    public List<Map<String,String>> getInitDates(String order) {
        List<Sdo> sdos=sdoDao.getInitDates(order);
        ...
        for (int i = 0; i < sdos.size(); i++) {
            ...
            map.put("iconPaht", temp.getIconPath());
            map.put("publisherPublishTime", simpleDateFormat.format(temp.getPublisherPublishTime()));
            map.put("publishOrganization", temp.getPublisher().getOrganization());
    ...
    public JSONObject getData(String prodId, String searchKey, String tags, String dataFormat, String publisher, String timerange,
        ...
        */
        ...
        List<Sdo> sdoList = Lists.newArrayList();
        ...
        for(SearchHit hit : top_200.getHits().getHits()){
            Map<String,Object> map = hit.getSourceAsMap();
            Sdo sdo = sdoDao.getSdoById(map.get("sdoid").toString());
            sdoList.add(sdo);
        }
